...
// Data Dictionary untuk relasi Model dan Varian
const carData = {
...
const PenawaranModal = ({ isOpen, onClose, selectedModel, selectedVarian }) => {
  ...
  // Mengatur nilai awal dropdown saat modal terbuka
  useEffect(() => {
    if (isOpen) {
      // Validasi apakah model yang dikirim dari props ada di list
      const initialModel = carData[selectedModel] ? selectedModel : 'Pilih Model';
      
      // Validasi apakah varian yang dikirim ada di dalam model tersebut
  ...
  // Jika modal ditutup, jangan render UI-nya
  ...
  // Handler KHUSUS untuk Model (Agar varian me-reset menyesuaikan model baru)
  const handleModelChange = (e) => {
    const newModel = e.target.value;
    setFormData({
      ...formData,
      model: newModel,
      varian: 'Pilih Varian' // Reset varian setiap kali ganti mobil
    });
  };
  ...
  const handleWhatsAppSubmit = (e) => {
    e.preventDefault();
    const noWA = '6281212345678'; // Ganti dengan nomor Sales Anda
  ...
  // Daftar varian yang muncul tergantung dari model yang dipilih saat ini
      ...
      <div className="bg-white rounded-xl w-full max-w-md max-h-[90vh] overflow-y-auto shadow-2xl relative animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="p-5 space-y-5">
          ...
          <form onSubmit={handleWhatsAppSubmit} className="space-y-4">
            ...
            {/* REVISI: Model dan Varian menjadi Dropdown Bersarang */}
            <div className="flex space-x-4">
              <div className="w-1/2">
